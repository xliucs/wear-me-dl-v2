#!/usr/bin/env python3
"""
V14: Optuna LGB + Feature Selection + GBR.

Research insight: waist circumference is #1 predictor of IR but we don't have it.
We're limited to BMI as body composition proxy. Focus on squeezing max from what we have.

Strategy:
1. Optuna-tune LGB with sqrt(y) weights (60 trials)
2. Optuna-tune GBR with sqrt(y) weights (30 trials) 
3. Feature selection: try top-K features by mutual info
4. Mega-blend with V13 Optuna XGB + new models
"""
...
t_start = time.time()
print("="*60)
print("  V14: OPTUNA LGB + FEATURE SELECTION + GBR")
print("="*60)
sys.stdout.flush()
...
# ============================================================
print("\n--- Part 1: Feature Selection ---"); sys.stdout.flush()
mi = mutual_info_regression(X_v7, y, random_state=42)
mi_ranking = sorted(enumerate(mi), key=lambda x: -x[1])
print("  Top 20 features by mutual info:")
for rank, (idx, score) in enumerate(mi_ranking[:20], 1):
    print(f"    {rank:2d}. {v7_cols[idx]:40s} MI={score:.4f}")
sys.stdout.flush()
...
for k in [15, 25, 35, 50]:
    top_idx = [idx for idx, _ in mi_ranking[:k]]
    X_topk = X_v7[:, top_idx]
    add(f'xgb_optuna_wsqrt_top{k}',
        get_oof(lambda: xgb.XGBRegressor(**v13_params, random_state=2024, verbosity=0),
                X_topk, y, splits, target_fn=log_fn, inv_fn=inv_log, weights=w_sqrt))
...
# ============================================================
# PART 2: OPTUNA LGB WITH WEIGHTS (50 trials)
# ============================================================
print("\n--- Part 2: Optuna LGB Weighted ---"); sys.stdout.flush()
...
def lgb_objective(trial):
    ...
    oof = get_oof(lambda: lgb.LGBMRegressor(**params), X_v7, y, splits,
                  target_fn=log_fn, inv_fn=inv_log, weights=w_sqrt)
    r2 = r2_score(y, oof)
    print(f"    LGB Trial {trial.number+1:3d}: R²={r2:.4f} (d={params['max_depth']}, lr={params['learning_rate']:.4f}, "
          f"n={params['n_estimators']}, leaves={params['num_leaves']})")
    sys.stdout.flush()
    return r2

study_lgb = optuna.create_study(direction='maximize', sampler=optuna.samplers.TPESampler(seed=42))
study_lgb.optimize(lgb_objective, n_trials=50, timeout=250)
lgb_best = study_lgb.best_params
lgb_best_r2 = study_lgb.best_value
print(f"\n  ★ LGB Optuna best: R²={lgb_best_r2:.4f}")
sys.stdout.flush()

add('lgb_optuna_wsqrt_v7',
    get_oof(lambda: lgb.LGBMRegressor(**{**lgb_best, 'verbose': -1, 'random_state': 42}),
            X_v7, y, splits, target_fn=log_fn, inv_fn=inv_log, weights=w_sqrt))

# ============================================================
# PART 3: OPTUNA GBR (sklearn) WITH WEIGHTS (30 trials)
# ============================================================
print("\n--- Part 3: Optuna GBR Weighted ---"); sys.stdout.flush()
...
def gbr_objective(trial):
    ...
    oof = get_oof(lambda: GradientBoostingRegressor(**params), X_v7, y, splits,
                  target_fn=log_fn, inv_fn=inv_log, weights=w_sqrt)
    r2 = r2_score(y, oof)
    print(f"    GBR Trial {trial.number+1:3d}: R²={r2:.4f} (d={params['max_depth']}, lr={params['learning_rate']:.4f})")
    sys.stdout.flush()
    return r2

study_gbr = optuna.create_study(direction='maximize', sampler=optuna.samplers.TPESampler(seed=42))
study_gbr.optimize(gbr_objective, n_trials=30, timeout=200)
gbr_best = study_gbr.best_params
gbr_best_r2 = study_gbr.best_value
print(f"\n  ★ GBR Optuna best: R²={gbr_best_r2:.4f}")
sys.stdout.flush()

add('gbr_optuna_wsqrt_v7',
    get_oof(lambda: GradientBoostingRegressor(**{**gbr_best, 'random_state': 42}),
            X_v7, y, splits, target_fn=log_fn, inv_fn=inv_log, weights=w_sqrt))
...
print(f"\n{'='*60}")
print(f"  V14 SUMMARY ({elapsed:.0f}s)")
print(f"{'='*60}")
print(f"  ★ BEST: R²={best_overall:.4f}")
print(f"  Best single: {sorted_m[0]} R²={scores[sorted_m[0]]:.4f}")
print(f"  LGB Optuna: R²={lgb_best_r2:.4f}, GBR Optuna: R²={gbr_best_r2:.4f}")
print(f"  Target: 0.65 | Gap: {0.65-best_overall:.4f}")

results = {
    'best_r2': float(best_overall),
    'lgb_optuna': float(lgb_best_r2), 'lgb_params': lgb_best,
    'gbr_optuna': float(gbr_best_r2), 'gbr_params': gbr_best,
    'best_single': {'name': sorted_m[0], 'r2': float(scores[sorted_m[0]])},
    'all_scores': {k: float(scores[k]) for k in sorted_m},
    'elapsed': elapsed
}
with open('v14_results.json','w') as f:
    json.dump(results,f,indent=2)
print(f"  Saved to v14_results.json"); sys.stdout.flush()
